...
execute anchored eyes run summon minecraft:wither_skull ^ ^ ^ {power: [0.0d, 0.0d, 0.0d], direction: [0.0d, 0.0d, 0.0d], pickup: 1b, damage: 4d, Tags: ["ptest", "p"], Color: -1, OwnerUUIDLeast: 0, OwnerUUIDMost: 0}
...
scoreboard players set @e[tag=ptest,tag=!p_setup,distance=..3,sort=nearest,limit=5] pP 100
#to copy your UUID to the Entity, use this function and target the spawned entities:
execute at @e[tag=ptest,tag=!p_setup,distance=..3,limit=1] run function p:uuid
execute as @e[tag=ptest,tag=!p_setup,distance=..3,limit=1] run function p:direction
execute as @e[tag=ptest,tag=!pow_setup,distance=..3,limit=1] run function p:power
