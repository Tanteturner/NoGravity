...
execute as @e[distance=..4,type=#game:force_pushable,type=!#game:projectiles] at @s anchored feet facing entity @p[tag=force_pushing] feet run summon minecraft:area_effect_cloud ^ ^0.4 ^-1.5 {Tags:["p_anchor"]}
execute as @e[distance=..4,type=#game:projectiles] at @s anchored eyes facing entity @e[type=#game:monsters,sort=nearest,limit=1] eyes run summon minecraft:area_effect_cloud ^ ^ ^1.5 {Tags:["p_anchor"]}
